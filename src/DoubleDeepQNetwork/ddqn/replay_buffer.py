...
class ReplayBuffer:
    ...
    def update_memory(self,state,next_state,action,reward,terminal):
        ...
        idx = self.memories_counter % self,self.max_memories
        ...
        self.terinal_mem[idx] = torch.tensor(terminal,dtype = self.terminal_mem.dtype)

        self.memories_counter += 1

    def access_memory(self,batch_size:int) -> Dict[str,torch.Tensor]:

        assert batch_size < self.max_memories, "Batch size is more than the maximum memories we can hold at once"
